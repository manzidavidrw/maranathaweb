...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const anchors = navLinks.filter((l) => l.type === "anchor").map((l) => l.href.replace("#", ""));
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border">
      ...
      <div className="hidden md:flex items-center justify-end bg-primary/10 px-6 lg:px-8 py-1.5">
        <a href="https://imaranatha.rw/webmail" target="_blank" rel="noopener noreferrer"
      ...
      <div className="container-max px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) =>
              link.type === "route" ? (
                <Link key={link.href} to={link.href}
                  className={`text-sm font-body font-medium transition-colors ${isActive(link) ? activeCls : inactiveCls}`}>
                  {link.label}
                </Link>
              ) : (
                <button key={link.href} onClick={() => handleAnchorClick(link.href)}
                  className={`text-sm font-body font-medium transition-colors ${isActive(link) ? activeCls : inactiveCls}`}>
                  {link.label}
                </button>
              )
            ...
            <Link to="/join">
              <Button variant="hero" size="lg">Join MFSC</Button>
            </Link>
          </div>
          ...
          <div className="md:hidden pb-4 border-t border-border mt-2 pt-4 space-y-1">
            {navLinks.map((link) =>
              link.type === "route" ? (
                <Link key={link.href} to={link.href} onClick={() => setIsOpen(false)}
                  className={`block py-2.5 px-3 rounded-lg text-sm font-body font-medium transition-colors ${isActive(link) ? mobileActiveCls : mobileInactiveCls}`}>
                  {link.label}
                </Link>
              ) : (
                <button key={link.href} onClick={() => handleAnchorClick(link.href)}
                  className={`block w-full text-left py-2.5 px-3 rounded-lg text-sm font-body font-medium transition-colors ${isActive(link) ? mobileActiveCls : mobileInactiveCls}`}>
                  {link.label}
                </button>
              )
            )}
            ...
            <div className="pt-2">
              <Link to="/join" onClick={() => setIsOpen(false)}>
                <Button variant="hero" className="w-full">Join MFSC</Button>
              </Link>
            </div>
          </div>
        )}
